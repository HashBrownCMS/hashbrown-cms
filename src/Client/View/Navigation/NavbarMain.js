...
class NavbarMain extends Crisp.View {
    constructor(params) {
        ...
        HashBrown.Service.EventService.on('resource', 'navbar', () => { this.reload(); });  
        HashBrown.Service.EventService.on('route', 'navbar', () => { this.updateHighlight(); });  
     ...
     *
     * @return {HashBrown.View.Navigation.NavbarPane} Pane
     */
    getCurrentPaneInstance() {
        return Crisp.View.get(HashBrown.View.Navigation.NavbarPane);
    }
    ...
    getPanes() {
        ...
        for(let name in HashBrown.View.Navigation) {
            let pane = HashBrown.View.Navigation[name];
         
            if(pane.scope && !HashBrown.Context.user.hasScope(pane.scope)) { continue; }

            if(pane.prototype instanceof HashBrown.View.Navigation.NavbarPane) {
                panes.push(pane);
            }
        }
