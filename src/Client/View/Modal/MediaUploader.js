 ...
 */
class MediaUploader extends HashBrown.View.Modal.Modal {
    /**
     * Constructor
     */
    constructor(params) {
        ...
        HashBrown.Service.MediaService.checkMediaProvider()
        .catch((e) => {
            UI.errorModal(e);
    ...
    async onSubmit(files) {
        ...
        try {
            ...
            for(let file of files) {
                let fileData = {
                    filename: file.name,
                    base64: await HashBrown.Service.MediaService.convertFileToBase64(file)
                };

                apiData.files.push(fileData);
            }

            let ids = await HashBrown.Service.RequestService.request('post', apiPath, apiData);

            if(this.folder && this.folder !== '/') {
                for(let id of ids) {
                    await HashBrown.Service.RequestService.request('post', 'media/tree/' + id, { id: id, folder: this.folder });
                }
            }
