...
_.div({class: 'resource-editor__header'},
    ...
    _.if(state.tabs && Object.keys(state.tabs).length > 0,
        _.div({class: 'resource-editor__header__tabs'},
            _.each(state.tabs, (alias, title) => 
                _.a({href: `#/${state.category}/${state.id}/${alias}`, class: `resource-editor__header__tab ${state.tab === alias ? 'active' : ''}`, onclick: (e) => _.onClickTab(alias)}, title)
            )
        ),
