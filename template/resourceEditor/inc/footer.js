...
state.tab !== 'overview' ? [
    _.div({class: 'resource-editor__footer'},
        ...
        _.div({class: 'resource-editor__footer__actions'},
            model ? [
                _.a({localized: true, href: `#/${state.library}/${state.id}/json`, class: 'widget widget--button embedded hidden-phone'}, 'Advanced'),
                !model.isLocked ? [
                ...
                _.button({localized: true, class: 'widget widget--button', onclick: _.onClickSaveSettings}, 'Save')
            ] : null
        )
    )
] : null
