...
(function e(t, n, r) {
  function s(o, u) {
    ...
    var IssuesEditor = require('./views/IssuesEditor');
    ...
    Router.route('/github/', function () {
      ...
      $('.workspace').html(_.div({ class: 'dashboard-container' }, [_.div({ class: 'panel panel-login' }, [_.div({ class: 'panel-heading' }, _.div({ class: 'login-icon' }, _.span({ class: 'fa fa-github' }))), _.div({ class: 'panel-body' }, _.form([_.input({ type: 'text', name: 'usr', class: 'form-control', placeholder: 'Username' }), _.input({ type: 'password', name: 'pwd', class: 'form-control', placeholder: 'Password' }), _.input({ type: 'submit', class: 'btn btn-primary', value: 'Log in' })]).on('submit', onSubmit))])]));
    });

    Router.route('/github/issues/', function () {
      var issuesEditor = new IssuesEditor({
  ...
  }, { "./views/IssuesEditor": 10 }], 10: [function (require, module, exports) {
    'use strict';
      ...
      _createClass(IssuesEditor, [{
        ...
        value: function sortIssues() {
          ...
          this.$element.find('.issue').each(function (i) {
            ...
            var $milestone = view.$element.find('.milestone[data-id="' + milestoneId + '"] .milestone-issues');

            var parentNumber = $issue.attr('data-parent');
            var $parentContainer = view.$element.find('.issue[data-number="' + parentNumber + '"] >.issue-children');
        ...
        value: function renderMilestone(milestone) {
          var $milestone = _.div({ class: 'milestone', 'data-id': milestone.id }, _.div({ class: 'panel' }, [_.div({ class: 'panel-heading' }, _.h4({ class: 'panel-title' }, milestone.title)), _.div({ class: 'panel-body milestone-issues' })]));
        ...
        value: function render() {
          ...
          this.$element.append(_.each(this.model, function (i, issue) {
            ...
            }, [_.div({ class: 'panel panel-primary' }, [_.div({ class: 'panel-heading' }, _.h4({ class: 'panel-title' }, issue.title)), _.div({ class: 'panel-body' }, markdownToHtml(body))]), _.div({ class: 'issue-children' })]);
